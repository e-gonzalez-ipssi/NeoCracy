...
class UserManager extends Manager {
    ...
    public function createUser(string $nom, string $prenom, string $mdp, string $mail): array {
        /** @var string $newQuery */
        $newQuery = "INSERT INTO `User` (`nom`, `prenom`, `mdp`, `mail`) VALUES ('$nom', '$prenom', '$mdp', '$mail')";
    ...
    public function getUserById(int $id): User {
        ...
        $newQuery = "SELECT `id`, `nom`, `prenom`, `mail`, `tel`, `photo`, `isAdmin` FROM `User` WHERE id = $id";
        $this->setQuery($newQuery);
    ...
    public function getUserByName(string $nom): array {
        $newQuery = "SELECT `id`, `nom`, `prenom`, `mail`, `tel`, `photo`, `isAdmin` FROM `User` WHERE nom = $nom";
        $this->setQuery($newQuery);
    ...
    public function getUserByMail(string $mail): User {
        $newQuery = "SELECT `id`, `nom`, `prenom`, `mail`, `tel`, `photo`, `isAdmin` FROM `User` WHERE mail = '$mail'";
        $this->setQuery($newQuery);
    ...
    public function getUserByToken(string $token): User {
        $newQuery = "SELECT `id`, `nom`, `prenom`, `mail`, `tel`, `photo`, `isAdmin` FROM `User` WHERE token = '$token'";
        $this->setQuery($newQuery);
    ...
    public function setUserToken(string $token, int $id): array {
        ...
        $newQuery = "UPDATE `User` SET token = '$token' WHERE id = $id";
        $this->setQuery($newQuery);
        $this->querySet();
    ...
    public function getPasswordById(int $id): string {
        $newQuery = "SELECT `mdp` FROM `User` WHERE id = $id";
        $this->setQuery($newQuery);
    ...
    public function getAllUserMail(): array {
        $newQuery = "SELECT `mail` FROM `User`";
        $this->setQuery($newQuery);
    ...
    public function deleteUserById(int $id): array {
        /** @var string $newQuery */
        $newQuery = "DELETE FROM `User` WHERE id = $id";
        $this->setQuery($newQuery);
        $this->querySet();
